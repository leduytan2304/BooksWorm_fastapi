...
def search_books(
    ...
    try:
        ...
        # Simplified query for substring matching in book_title and author_name
        sql_query = text("""
            SELECT 
                b.id,
                b.book_title,
            ...
            JOIN author a ON b.author_id = a.id
            WHERE (
                LOWER(b.book_title) LIKE LOWER(:search_pattern) OR
                LOWER(a.author_name) LIKE LOWER(:search_pattern)
            )
            ...
            HAVING COALESCE(AVG(r.rating_star::FLOAT), 0) >= :star_value
            ORDER BY 
                CASE WHEN LOWER(b.book_title) LIKE LOWER(:search_pattern) THEN 0 ELSE 1 END,
                b.book_title ASC
            OFFSET :offset
            LIMIT :limit
        """)
        ...
        print(f"Search returned {len(books)} results")
